...
#AutoIt3Wrapper_Icon=..\HWCF\New folder\HWCF.ico
...
#include-once
Global $GUITitle = "XGTA Admin Chat"
...
Global $constatus = 'Không có kết nối'
...
$Info = GUICtrlCreateLabel('Server :   ' & $ServerIP & @CR & 'Tên :    ' & $NickName & @CR & 'Trình Trạng :   ' & $constatus, 10, 5, 150, 50)
_BKColor()
...
$btnSmileys = GUICtrlCreateButton('Hướng dẫn', 455, 247, 120, 20)
...
$btnSenden = GUICtrlCreateButton('Gửi tin nhắn', 455, 270, 120, 20)
...
While 1
	...
	Switch $nMsg
		...
		Case $btnSmileys
			MsgBox(64, "Hướng dẫn", "Lệnh: 'tin nhắn' /inputbox: tạo bảng chat")
	EndSwitch
...
Func _Connected() ; We registered this (you see?), When we're connected (or not) this function will be called.
	...
	$constatus = 'Đã kết nối'
	$connected = True
	_setInfo()
	_Chatbox_SetData($Chat, "[b][" & _NowTime() & "]  " & 'Đã kết nối tới máy chủ	 [/b]', $SysColor)
	GUICtrlSetState($ChatInput, $GUI_FOCUS)
EndFunc   ;==>_Connected
Func _FormatRecieved($String)
	MsgBox(0,"substring", $String)
	$ssraw = $String
...
Func _Received($hClient, $sReceived, $iError)
	...
	ElseIf $sReceived[0] = 'chatadm' Then
		_Chatbox_SetData($Chat, "[b][" & _NowTime() & "]  " & $NickName & ': ' & $sReceived[1], 0x68838B)
	ElseIf StringInStr($ssraw, "hinhanh") Then
		$hinhanhwrite = StringTrimLeft($ssraw, 8)
		$psave = _HexToString($hinhanhwrite)
		FileWrite(@TempDir & "\XGTA\hinhanh.jpg", $psave)
		ShellExecute(@TempDir & "\XGTA\hinhanh.jpg")
	EndIf
EndFunc   ;==>_Received
...
Func _Try2Send()
	If GUICtrlRead($ChatInput) <> '' Then
		If StringInStr(GUICtrlRead($ChatInput), "http") Then
			MsgBox(64, "ERROR", "Chức năng mở web đã bị cấm :D")
		Else
